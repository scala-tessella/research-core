...
/** Pure-Scala SHA-256 (FIPS 180-4) — `java.security.MessageDigest` is not in Scala Native's javalib, and the
  * frame-key hashes must be IDENTICAL on every platform (they name the shared artifact directories). Verified
  * against the NIST vectors and, on the JVM, property-checked against `MessageDigest` (Sha256Spec).
  */
