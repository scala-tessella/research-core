...
/** Fast teeth for the [[K1Certify]] encoding, mirroring `K2CertifySpec`: the SAT enumeration must equal, op
  * for op, the BFS relabelings of the single-vertex-orbit slice of the relaxed universe generator — two
  * independent enumerators, one universe. K1's encoding has no tier-1 layer, so the oracle runs
