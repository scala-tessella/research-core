...
/** End-to-end teeth for the fs2-io/Cats Effect external-process layer of [[CertifyRunner]] — the plumbing
  * the DRAT certification harness stands on. Cancelled (not failed) when `tools/bin/{kissat,drat-trim}` are
  * not installed, like the guarded probes.
  */
class CertifyRunnerSpec extends AnyFlatSpec with Matchers:
  ...
  "certifyCnf (fs2-io external processes)" should "certify a trivially UNSAT instance" in:
    requireTools()
    val dir                         = Files.createTempDirectory("certify-cnf")
  ...
  it should "not report UNSAT on a satisfiable instance" in:
    requireTools()
    val dir             = Files.createTempDirectory("certify-cnf-sat")
  ...
  "certifyFrame" should "fully certify the 4⁴ frame sweep end-to-end" in:
    ...
    val dir    = Files.createTempDirectory("certify-frame")
