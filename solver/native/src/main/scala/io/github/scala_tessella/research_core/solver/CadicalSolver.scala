...
/** The Scala Native [[SatSolver]]: CaDiCaL in-process through IPASIR — no process spawn, no DIMACS
  ...
  * the certified one. `timeoutSeconds` is enforced through the IPASIR terminate callback (the deadline
  * lives in malloc'd state, because a C function pointer cannot capture); expiry surfaces as
  * [[SatSolver.Timeout]], the counterpart of SAT4J's `TimeoutException` on the JVM.
  */
