...
/** Regression teeth for `searchPatterns`'s `capped` flag over the real species corpus. Ground truth for "the
  * cap actually truncated the search" is a re-run at cap + 1 finding more patterns. A false NEGATIVE
  ...
  * backtracking branch never set the flag, and real cap-hit unwinds pass only through it). A false POSITIVE
  * (complete but reported capped) merely wastes a re-run at a higher cap and is reported here
  * informationally.
  */
class CappedProbeSpec extends AnyFlatSpec with Matchers:

  "searchPatterns.capped" should
    "never report a truncated search as uncapped (species corpus, caps 1 and 40)" in:
      val flags    = Flags()
      var checked  = 0
      var falseNeg = 0
      var falsePos = 0
      ...
      for idx <- SpeciesEnumerator.species.indices do
        ...
        for (domains, si) <- acc.skeletons.zipWithIndex; cap <- List(1, 40) do
          ...
          if capped != truncated then
            ...
            rows +=
              s"species $idx skeleton $si cap $cap: found=$found reportedCapped=$capped actuallyTruncated=$truncated"
      rows.result().foreach(info(_))
      info(
        s"checked $checked (species, skeleton, cap) cases: $falseNeg false negatives, $falsePos false positives"
      )
      checked should be > 0
      falseNeg shouldBe 0
